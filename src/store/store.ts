...
export const useMeasureConversionStore = create<MeasureConversionStore>()(
  persist(
    (set) => ({
      converters: [
        defaultConverter({
      ...
      removeConverter: (id) => {
        set((state) => {
          if (state.converters.length <= 1) return state;
